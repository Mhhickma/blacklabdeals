...
def static_product_card(deal: dict) -> str:
    card_title = esc(title(deal))
    link = esc(deal.get("link") or deal.get("amazon_url") or deal.get("url") or "#")
    brand = esc(deal.get("brand") or "")
    cat_label = esc(category(deal))
    price = esc(deal.get("price") or money(price_amount(deal)) or "See current price on Amazon")
    updated = esc(deal.get("updated_at") or deal.get("seen_at") or "the current Amazon feed")
    separator = '<span aria-hidden="true">·</span>' if brand and cat_label else ""
    return f'''<article class="product-card">
        <a class="product-card-link" href="{link}" target="_blank" rel="nofollow sponsored noopener">
          <div class="product-card-image">{deal_image(deal)}</div>
          <div class="product-card-body">
            <div class="product-card-meta">{brand}{separator}{cat_label}</div>
            <h3 class="product-card-title">{card_title}</h3>
            <div class="product-card-price"><span>Current Amazon price</span><strong>{price}</strong></div>
            <p class="product-card-time">Product information shown as of {updated}.</p>
...
def homepage_deal_card(deal: dict) -> str:
    return static_product_card(deal)
    card_title = esc(title(deal))
    link = esc(deal.get("link") or deal.get("amazon_url") or deal.get("url") or "#")
    cat_label = esc(category(deal))

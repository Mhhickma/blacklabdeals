...
(function(){
  ...
  function addStyles(){
    ...
    s.textContent += '.bld-menu::after{content:"";position:absolute;left:-18px;right:-18px;top:100%;height:20px;background:transparent;pointer-events:auto}.bld-menu-panel{top:calc(100% + 2px)}.bld-menu:hover .bld-menu-btn,.bld-menu:focus-within .bld-menu-btn,.bld-menu.is-open .bld-menu-btn{border-color:#9db5ca;color:#1a3a5c;box-shadow:0 0 0 3px rgba(26,58,92,.08)}.bld-menu.is-open .bld-menu-panel{display:grid;grid-template-columns:1fr 1fr;gap:4px}@media(max-width:820px){.bld-menu.is-open .bld-menu-panel{grid-template-columns:1fr}}';
